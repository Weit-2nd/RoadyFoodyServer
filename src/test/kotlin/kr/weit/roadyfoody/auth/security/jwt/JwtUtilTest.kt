...
class JwtUtilTest : BehaviorSpec({
    ...
    every { jwtProperties.accessKey } returns generateHmac256Key()
    every { jwtProperties.refreshKey } returns generateHmac256Key()
    every { jwtProperties.accessTokenExpirationTime } returns 1000 * 60 * 10
    every { jwtProperties.refreshTokenExpirationTime } returns 1000 * 60 * 60
    ...
    given("generateAccessToken 메서드") {
        `when`("유효한 userId 를 전달하면") {
            then("AccessToken 이 생성된다") {
                val actual = jwtUtil.generateAccessToken(TEST_USER_ID)
    ...
    given("generateRefreshToken 메서드") {
        `when`("유효한 userId 와 rotateId 를 전달하면") {
            then("RefreshToken 이 생성된다") {
                val actual = jwtUtil.generateRefreshToken(TEST_USER_ID, TEST_ROTATE_ID)
                actual.shouldNotBeNull()
            }
        }
    }
    ...
    given("storeCachedRefreshTokenRotateId 메서드") {
        `when`("유효한 userId 와 rotateId 를 전달하면") {
            ...
            then("Redis 에 rotateId 가 저장된다") {
                jwtUtil.storeCachedRefreshTokenRotateId(TEST_USER_ID, TEST_ROTATE_ID)
                verify(exactly = 1) {
    ...
    given("getRefreshTokenCacheKey 메서드") {
        `when`("유효한 userId 를 전달하면") {
            then("RefreshTokenCacheKey 가 생성된다") {
                val actual = JwtUtil.getRefreshTokenCacheKey(TEST_USER_ID)
                actual.shouldNotBeNull()
            }
        }
    }
    ...
    given("validateCachedRefreshTokenRotateId 메서드") {
        `when`("유효한 RefreshToken 을 전달하면") {
            ...
            val refreshToken = jwtUtil.generateRefreshToken(TEST_USER_ID, jwtUtil.generateRotateId())
    ...
    given("validateToken 메서드") {
        `when`("유효한 토큰을 전달하면") {
            then("true 가 반환된다") {
                forAll(
                    row(jwtUtil.accessKey, jwtUtil.generateAccessToken(TEST_USER_ID)),
                    row(jwtUtil.refreshKey, jwtUtil.generateRefreshToken(TEST_USER_ID, jwtUtil.generateRotateId())),
                ) { key, token ->
        ...
        `when`("만료기간이 지난 토큰을 전달하면") {
            then("false 가 반환된다") {
                val accessToken = jwtUtil.generateAccessToken(TEST_USER_ID, expirationTime = -10000L)
